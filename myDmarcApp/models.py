...
class View(OrderedModel):
    ...
    def getTableData(self, records=None):
        ...
        return [[r.report.reporter.org_name,
                ...
                '<br>'.join(["%s (%s)" % (dkim.domain, dkim.get_result_display()) for dkim in r.authresultdkim_set.all()]),
                '<br>'.join(["%s (%s)" % (spf.domain, spf.get_result_display()) for spf in r.authresultspf_set.all()]),
                r.count,
                r.source_ip,
                r.country_iso_code,
